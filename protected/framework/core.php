...
try {
	...
	if( defined('_XHPROF_OPEN') && _XHPROF_OPEN && !empty($GLOBALS['app_id'])){
		$xhprof_data = xhprof_disable();  
		$xhprof_data = serialize($xhprof_data);
		$source = $GLOBALS['app_id'];
...
function var_log($msg, $label=''){
	return obj('dwLog')->write($msg, $label);
}
...
function __template_page($params) {
	...
	$page_str="<span>共{$page['total_count']}条记录  共{$page['total_page']}页</span>";
